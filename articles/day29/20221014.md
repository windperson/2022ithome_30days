...
在Client端和Silo端各自的Builder程式碼在配置時，通常都會有 `Configure<ClusterOptions>()` 的擴充方法呼叫，如下範例：
   ...
   ```
   這兩個設定值的意義是：
   * `ClusterId`：代表Orleans Cluster之中此節點的唯一識別名稱，不可和Cluster之中其他節點重複。
   * `ServiceId`：代表Orleans Cluster的服務識別名稱，通常會使用應用程式名稱來當作ServiceId，例如：MyAwesomeApp、MySuperApp等等。

要記住的重點是：**同個ServiceId的Client-to-Silo, Silo-to-Silo的RPC呼叫才能連結的到**。在運營時，Orleans Cluster的節點可以動態增減，就是利用這兩個設定值來確認Client端和Silo端是否可以互相連結的到，以及區隔不同種類的服務。

如果是使用 [ASP.NET Core Co-hosting](https://learn.microsoft.com/en-us/dotnet/orleans/host/client#co-hosted-clients) 這種RPC呼叫端和Grain執行端都在同個Process的配置時，可以不必設定此 [`ClusterOption`](https://learn.microsoft.com/en-us/dotnet/api/orleans.configuration.clusteroptions) 設定。
